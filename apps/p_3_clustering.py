...
features_df, X_scaled = select_cyber_features(df_brut)
relevant_cols = ['dur', 'sbytes', 'dbytes', 'sttl', 'dttl', 'spkts', 'dpkts', 'rate']
...
labels_hca, indices_hca = get_hca_clusters(X_scaled, n_clusters=9)

if labels_hca is not None:
    print("\n--- Répartition des points par Cluster (HCA) ---")
    print(pd.Series(labels_hca).value_counts().sort_index())

find_best_epsilon(X_scaled)

db_model = DBSCAN(eps=0.5, min_samples=10).fit(X_scaled)
labels_dbscan = db_model.labels_

bruit_df = isolate_noise_points(df_brut, labels_dbscan)